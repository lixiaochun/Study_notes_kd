    ...
    , ui(new Ui::MainWindow)
{
    ...
    ui->toolBar->addAction(ui->actAddFolder);
    ui->toolBar->addAction(ui->actAddFiles);
    ui->toolBar->addAction(ui->actDelItem);
    ui->toolBar->addAction(ui->actScanItems);
    ui->toolBar->addAction(ui->actZoomIn);
    ui->toolBar->addAction(ui->actZoomOut);
    ui->toolBar->addAction(ui->actZoomFitH);
    ui->toolBar->addAction(ui->actZoomFitW);
    ui->toolBar->addAction(ui->actZoomActualSize);
    ui->toolBar->addAction(ui->actDockVisible);
    ui->toolBar->addAction(ui->actDockFloat);
}
...
{ //初始化Tree
    ...
    QIcon icon;
    icon.addFile("./images/icons/home.png"); //设置ICON的图标
...
{//添加一个目录节点
    QIcon   icon("./images/icons/card.png");
...
{//添加图片文件节点
    ...
    item=ui->treeWidget->currentItem(); //当前节点
...
{//添加一个图片文件节点
    QIcon   icon("./images/icons/card.png");//ICON的图标
...
{//当前节点选择变化时触发
    ...
    switch(var)
    {
        case  itTopItem: //顶层节点
          ...
          ui->actDelItem->setEnabled(false);    //顶层节点不能删除
          break;
...
void MainWindow::on_actZoomActualSize_triggered()
{//实际大小显示
    pixRatio=1;  //恢复显示比例为1
