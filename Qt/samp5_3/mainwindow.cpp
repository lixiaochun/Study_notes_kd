    ...
    , ui(new Ui::MainWindow )
{
    ...
    connect(theSelection,SIGNAL(currentChanged(QModelIndex,QModelIndex)), this,SLOT(on_currgentChanged(QModelIndex,QModelIndex)));
    ui->tableView->setModel (theModel) ; //设置数据模型
    ui->tableView->setSelectionModel(theSelection) ; //设置选择模型
    ui->tableView->setSelectionMode(QAbstractItemView::ExtendedSelection);
    ui->tableView->setSelectionBehavior(QAbstractItemView::SelectItems);
    //创建状态栏组件
    LabCurFile = new QLabel(_T("当前文件："), this);
...
{ //选择单元格变化时的响应
    ...
    if (current.isValid())
    {
        LabCellPos->setText ( _T("当前单元格：") + QString::asprintf("%d", current.row()) + _T(" 行，")
                              + QString::asprintf("%d", current.column()) + _T(" 列") );
        QStandardItem* aItem = theModel->itemFromIndex(current);
        this->LabCellText->setText (_T("单元格内容：") + aItem->text());
...
{//打开文件
    ...
    //调用打开文件对话框打开一个文件
    QString aFileName=QFileDialog::getOpenFileName(this, _T("打开一个文件"),curPath, _T("井数据文件(*.txt);;所有文件(*.*)") );
    ...
    if (aFile.open(QIODevice::ReadOnly | QIODevice::Text)) //以只读文本方式打开文件
    {
        ...
        this->LabCurFile->setText(_T("当前文件：")+aFileName);//状态栏显示
...
{//保存为文件
    QString curPath=QCoreApplication::applicationDirPath(); //获取应用程序的路径
    //调用打开文件对话框选择一个文件
    QString aFileName=QFileDialog::getSaveFileName(this,_T("选择一个文件"),curPath,
                 _T("井斜数据文件(*.txt);;所有文件(*.*)") );
...
{//在表格内插入行
    ...
    curIndex=theModel->index(curIndex.row(),0);//创建插入行的ModelIndex
    theSelection->clearSelection();//清空选择项
    theSelection->setCurrentIndex(curIndex,QItemSelectionModel::Select);//设置刚插入的行为当前选择行
}
...
void MainWindow::on_acrAlignLeft_triggered()
{//设置文字居左对齐
...
void MainWindow::on_actAligntCenter_triggered()
{//设置文字居中对齐
    if (!theSelection->hasSelection())
        return;
    //获取选择的单元格的模型索引列表，可以是多选
...
void MainWindow::on_actAligntRight_triggered()
{//设置文字居右对齐
    if (!theSelection->hasSelection())
        return;
    //获取选择的单元格的模型索引列表，可以是多选
    ...
    for (int i=0;i<selectedIndex.count();i++)
    {
        QModelIndex aIndex=selectedIndex.at(i) ; //获取一个模型索引
        QStandardItem* aItem=theModel->itemFromIndex(aIndex);
        aItem->setTextAlignment(Qt::AlignRight|Qt::AlignVCenter) ;//设置文字对齐方式
    }
}
...
void MainWindow::on_actModleData_triggered()
{
    QStandardItem   *aItem;
    int i,j;
    QString str;
