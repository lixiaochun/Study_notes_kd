...
void MainWindow::on_treeView_clicked(const QModelIndex &index)
{
    ...
    ui->LabPath->setText(_T("路径名：") + model->filePath(index));
    ui->LabType->setText(_T("节点类型：") + model->type(index));
    ui->LabFileName->setText(_T("文件名：") + model->fileName(index));
    int sz=model->size(index)/1024;
    if (sz<1024)
        ui->LabFileSize->setText(_T("文件大小：") + QString("%1 KB").arg(sz));
    else
        ui->LabFileSize->setText(_T("文件大小：") + QString::asprintf("%.1f MB",sz/1024.0));
}
